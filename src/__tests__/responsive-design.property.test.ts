    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          fontScale: fc.float({ min: Math.fround(0.8), max: Math.fround(1.2) }).filter(x => !Number.isNaN(x)),
        }),
        (config) => {
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          batteryLevel: fc.float({ min: Math.fround(0.1), max: Math.fround(1.0) }).filter(x => !Number.isNaN(x)),
        }),
        (config) => {
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          paddingScale: fc.float({ min: Math.fround(0.5), max: Math.fround(2.0) }).filter(x => !Number.isNaN(x)),
          lineLength: fc.integer({ min: 45, max: 75 }), // Characters per line
          fontSizeRem: fc.float({ min: Math.fround(0.75), max: Math.fround(1.25) }).filter(x => !Number.isNaN(x)),
        }),
        (config) => {
